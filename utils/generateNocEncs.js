...
export default function generateNocEncs() {
  const zip = JSZip();

  for (const [_, value] of Object.entries(nocAmounts)) {
    zip.file(
      `EVT-MXE-0${eventNum} [P2024 Simplified].cfg`,
  ...
  zip.generateAsync({ type: "blob" }).then(function (content) {
    saveAs(content, "noc-encs.zip");
  });
}

const getConfigContents = (start, length) => {
  let currentContents = configBase;
...
[STREAM_${i + 1}]
ID=${i + 1}
Name=LDC_MXD_0${getTwoDigitNum(start + i)}
Contents=3
VideoSource=${i}
AudioSource=${4 * (i + 1) - 4},${4 * (i + 1) - 3},${4 * (i + 1) - 2},${
        4 * (i + 1) - 1
      }
TOS=128
MTU=1496
TTL=64
Port=${5 + ((start + i + 1) % 2)}0${getTwoDigitNum(start + i)}
...
[STREAM_${z + length + 1}]
ID=${z + length + 1}
Name=NHN_MXD_0${getTwoDigitNum(start + z)}
Contents=3
VideoSource=${z}
AudioSource=${4 * (z + 1) - 4},${4 * (z + 1) - 3},${4 * (z + 1) - 2},${
        4 * (z + 1) - 1
      }
TOS=128
MTU=1496
TTL=64
Port=${5 + ((start + z + 1) % 2)}0${getTwoDigitNum(start + z)}
